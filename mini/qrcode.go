...
func (sdk *SDK) CreateMiniSceneCode(ctx context.Context, param *WxMiniSceneParam) (req []byte, err error) {
	...
	bodyMap.Set("scene", param.Scene) //最大32个可见字符，只支持数字
	bodyMap.Set("width", param.Width)
	bodyMap.Set("page", param.Page)            //必须是已经发布的小程序存在的页面
	bodyMap.Set("auto_color", false)           //自动配置线条颜色，如果颜色依然是黑色，则说明不建议配置主色调
	bodyMap.Set("is_hyaline", param.IsHyaline) //是否需要透明底色
	bodyMap.Set("line_color", lineColorMap)
